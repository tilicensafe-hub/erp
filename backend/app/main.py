...
def _billing_for_user(billing_id: str, user: dict[str, Any] | None) -> dict[str, Any]:
    billing = fetch_one("select * from billing_invoices where id = %s and deleted_at is null limit 1", (billing_id,))
    if not billing:
...
def _delete_billing(billing_id: str) -> dict[str, Any]:
    billing = fetch_one("select id from billing_invoices where id = %s and deleted_at is null limit 1", (billing_id,))
    if not billing:
