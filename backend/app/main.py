...
def _sign_nfe_xml(xml_content: str, access_key: str) -> str:
    ...
    cert_chain.extend(cert.public_bytes(Encoding.PEM) for cert in (extra_certificates or []))
    signed = XMLSigner(
        method=methods.enveloped,
        signature_algorithm="rsa-sha1",
        digest_algorithm="sha1",
        c14n_algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315",
    ).sign(root, key=key_pem, cert=cert_chain, reference_uri=f"#{reference_id}")
...
def _send_signed_nfe_to_sefaz(signed_xml: str) -> str:
    try:
        ...
        raise RuntimeError(f"dependencia requests-pkcs12 ausente no Render: {error}") from error
    ...
    response = pkcs12_post(
        ...
        pkcs12_password=settings.nfe_certificate_password,
        timeout=90,
    )
