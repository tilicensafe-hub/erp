...
def issue_mobile_fiscal(number: int, payload: FiscalIssueIn | None = None, user: dict[str, Any] = Depends(require_seller_session)) -> dict[str, Any]:
    ...
    status_text = "blocked" if not fiscal_status["ready"] else "draft"
    xml_content = _mobile_nfe_xml(order, series, next_number, environment, random_code, access_key)
    message = (
        "Nota validada, mas a emissao SEFAZ online pelo Render ainda nao esta completa: "
        + "; ".join(fiscal_status["missing"])
        if not fiscal_status["ready"]
        else "XML da NF-e gerado na nuvem e aguardando envio SEFAZ."
    )
