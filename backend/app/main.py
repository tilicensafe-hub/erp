...
def deleted_customers() -> list[dict[str, Any]]:
    return fetch_all(
        """
        select id, local_id, name, document, updated_at, deleted_at
        from customers
...
@app.post("/orders/{number}/billing", dependencies=[Depends(require_token)])
def issue_order_billing(number: int) -> dict[str, Any]:
    try:
        return _issue_mobile_billing(
            number,
            {
                "id": None,
                "local_id": None,
                "name": "Windows",
                "email": "windows@licensafe.local",
                "role": "admin",
            },
        )
        ...
        raise HTTPException(status_code=400, detail=f"Falha ao emitir cobranca: {type(error).__name__}: {error}") from error
...
def download_mobile_fiscal_danfe(invoice_id: str, user: dict[str, Any] = Depends(require_seller_session)) -> Response:
    ...
    items = fetch_all(
        """
        select items.*, products.ncm, products.cfop, products.cst_csosn
...
def _delete_customer_by_identifier(customer_id: str, local_id: int | None = None, document: str | None = None, name: str | None = None, cleanup_duplicates: bool = False) -> dict[str, Any]:
    ...
    normalized_name = _normalize_name(name or raw_identifier)
...
def _save_customer(item: CustomerIn) -> dict[str, Any]:
    ...
    if item.active is False:
        return _delete_customer_by_identifier(item.id or item.document or item.name, item.local_id, item.document, item.name, True)
    existing = None
    ...
    if existing is None:
        existing = fetch_one(
            ...
            where regexp_replace(lower(trim(name)), '\\s+', ' ', 'g') = %s
              and deleted_at is null
            order by
              (regexp_replace(coalesce(document, ''), '\\D', '', 'g') <> '') desc,
    ...
    return fetch_all(
        ...
        select * from (
          ...
          from (
            select customers.*,
              case
                when regexp_replace(lower(trim(name)), '\\s+', ' ', 'g') <> ''
                  then 'name:' || regexp_replace(lower(trim(name)), '\\s+', ' ', 'g')
...
def _mobile_nfe_xml(order: dict[str, Any], series: int, number: int, environment: str, random_code: str, access_key: str) -> str:
    ...
    subtotal = round(
        sum(float(item["total"] or round(float(item["quantity"] or 0) * float(item["unit_price"] or 0), 2)) for item in rows),
        2,
    )
    discount = round(max(0, subtotal - total_value), 2)
    discount_cents = int(round(discount * 100))
    item_totals = [
        float(item["total"] or round(float(item["quantity"] or 0) * float(item["unit_price"] or 0), 2))
        for item in rows
    ]
    total_item_cents = max(1, int(round(sum(item_totals) * 100)))
    allocated_discounts: list[float] = []
    allocated_cents = 0
    for index, item_total in enumerate(item_totals):
        if discount_cents <= 0:
            cents = 0
        elif index == len(item_totals) - 1:
            cents = discount_cents - allocated_cents
        else:
            cents = int(round(discount_cents * int(round(item_total * 100)) / total_item_cents))
            allocated_cents += cents
        allocated_discounts.append(cents / 100)
    ...
    for index, item in enumerate(rows, start=1):
        ...
        item_discount = allocated_discounts[index - 1] if index - 1 < len(allocated_discounts) else 0
        items_xml.append(
            f"""
    <det nItem="{index}">
      <prod>
        ...
        {f'<vDesc>{_money(item_discount)}</vDesc>' if item_discount > 0 else ''}
        <indTot>1</indTot>
      </prod>
      <imposto>
        <ICMS>
...
def _sign_nfe_xml(xml_content: str, access_key: str) -> str:
    ...
    digest = hashlib.sha1(etree.tostring(inf_nfe, method="c14n", exclusive=True, with_comments=False)).digest()
    ...
    signed_info_c14n = etree.tostring(
        etree.fromstring(etree.tostring(signed_info)),
        method="c14n",
        exclusive=False,
        with_comments=False,
    )
...
def _mobile_danfe_pdf(
    ...
    def text(x: float, yy: float, value: Any, size: int = 7, bold: bool = False, max_chars: int = 90) -> None:
        pdf.setFont("Helvetica-Bold" if bold else "Helvetica", size)
        pdf.drawString(x, yy, str(value or "-")[:max_chars])

    def center(x: float, yy: float, width: float, value: Any, size: int = 7, bold: bool = False) -> None:
        pdf.setFont("Helvetica-Bold" if bold else "Helvetica", size)
        pdf.drawCentredString(x + (width / 2), yy, str(value or "-"))

    def right(x: float, yy: float, value: Any, size: int = 7, bold: bool = False) -> None:
        pdf.setFont("Helvetica-Bold" if bold else "Helvetica", size)
        pdf.drawRightString(x, yy, str(value or "-"))
    ...
    def box(x: float, yy: float, width: float, height: float, title: str = "") -> None:
        ...
        if title:
            text(x + 2 * mm, yy - 4 * mm, title.upper(), 5, True, 70)

    def label_box(x: float, yy: float, width: float, height: float, title: str, value: Any, size: int = 7, bold: bool = False) -> None:
        box(x, yy, width, height, title)
        text(x + 2 * mm, yy - height + 3 * mm, value, size, bold, 80)

    def wrapped(x: float, yy: float, value: Any, width_chars: int, size: int = 7, max_lines: int = 2) -> None:
        ...
        for index, line_text in enumerate(lines[:max_lines]):
            text(x, yy - (index * 3.5 * mm), line_text, size, False, width_chars)
    ...
    total = float(order.get("total") or 0) if order else 0
    subtotal = sum(float(item.get("total") or 0) for item in items)
    discount = max(0, subtotal - total)
    header_h = 36 * mm
    left_w = 76 * mm
    middle_w = 46 * mm
    right_w = usable - left_w - middle_w

    box(margin, y, left_w, header_h, "Identificacao do emitente")
    logo_path = os.path.join(os.path.dirname(__file__), "static", "licensafe.png")
    logo_space = 26 * mm if os.path.exists(logo_path) else 0
    if logo_space:
        try:
            pdf.drawImage(ImageReader(logo_path), margin + 3 * mm, y - 28 * mm, width=22 * mm, height=22 * mm, preserveAspectRatio=True, mask="auto")
        except Exception:
            logo_space = 0
    issuer_x = margin + logo_space + 2 * mm
    issuer_w = left_w - logo_space - 4 * mm
    text(issuer_x, y - 9 * mm, issuer, 8, True, 35)
    text(issuer_x, y - 14 * mm, settings.nfe_issuer_trade_name or issuer, 6, False, 35)
    wrapped(issuer_x, y - 20 * mm, settings.nfe_issuer_address, 39, 5, 2)
    text(issuer_x, y - 31 * mm, f"CNPJ: {doc_number(settings.nfe_issuer_cnpj)}", 5, False, 31)
    text(issuer_x + (issuer_w / 2), y - 31 * mm, f"IE: {_digits_only(settings.nfe_issuer_ie)}", 5, False, 28)

    box(margin + left_w, y, middle_w, header_h)
    center(margin + left_w, y - 9 * mm, middle_w, "DANFE", 17, True)
    center(margin + left_w, y - 15 * mm, middle_w, "Documento Auxiliar da", 6)
    center(margin + left_w, y - 19 * mm, middle_w, "Nota Fiscal Eletronica", 6)
    center(margin + left_w, y - 25 * mm, middle_w, "0 - ENTRADA", 6, True)
    center(margin + left_w, y - 29 * mm, middle_w, "1 - SAIDA", 6, True)
    center(margin + left_w, y - 34 * mm, middle_w, f"NF-e No {invoice.get('number')}  Serie {invoice.get('series')}", 7, True)

    box(margin + left_w + middle_w, y, right_w, header_h, "Controle do Fisco")
    if code128 and _digits_only(access_key):
        max_barcode_width = right_w - (8 * mm)
        barcode = code128.Code128(_digits_only(access_key), barHeight=10 * mm, barWidth=0.32 * mm)
        if barcode.width > max_barcode_width:
            barcode = code128.Code128(
                _digits_only(access_key),
                barHeight=10 * mm,
                barWidth=max(0.18 * mm, 0.32 * mm * (max_barcode_width / barcode.width)),
            )
        barcode_x = margin + left_w + middle_w + ((right_w - barcode.width) / 2)
        barcode.drawOn(pdf, barcode_x, y - 18 * mm)
    center(margin + left_w + middle_w, y - 26 * mm, right_w, _format_access_key_for_pdf(access_key), 7, True)
    center(margin + left_w + middle_w, y - 32 * mm, right_w, f"Ambiente: {str(invoice.get('environment') or '').upper()}", 6, False)
    y -= header_h

    line_h = 11 * mm
    label_box(margin, y, 118 * mm, line_h, "Natureza da operacao", "Venda", 7, True)
    label_box(margin + 118 * mm, y, usable - 118 * mm, line_h, "Protocolo de autorizacao de uso", invoice.get("protocol") or "-", 7, True)
    y -= line_h
    label_box(margin, y, 42 * mm, line_h, "Inscricao Estadual", _digits_only(settings.nfe_issuer_ie))
    label_box(margin + 42 * mm, y, 42 * mm, line_h, "IE Subst. Tributario", "-")
    label_box(margin + 84 * mm, y, 42 * mm, line_h, "CNPJ", doc_number(settings.nfe_issuer_cnpj))
    label_box(margin + 126 * mm, y, usable - 126 * mm, line_h, "Chave de acesso", _format_access_key_for_pdf(access_key), 6, True)
    y -= line_h + 4 * mm

    text(margin, y, "DESTINATARIO / REMETENTE", 7, True)
    y -= 2 * mm
    dest_h = 24 * mm
    box(margin, y, usable, dest_h)
    text(margin + 2 * mm, y - 5 * mm, "NOME / RAZAO SOCIAL", 5, True)
    text(margin + 2 * mm, y - 10 * mm, customer.get("name") if customer else "-", 8, True, 62)
    text(margin + 112 * mm, y - 5 * mm, "CNPJ / CPF", 5, True)
    text(margin + 112 * mm, y - 10 * mm, doc_number(customer.get("document") if customer else ""), 7, True, 25)
    text(margin + 152 * mm, y - 5 * mm, "DATA DA EMISSAO", 5, True)
    text(margin + 152 * mm, y - 10 * mm, _format_dt_for_pdf(invoice.get("created_at")), 7, True, 20)
    text(margin + 2 * mm, y - 17 * mm, "ENDERECO", 5, True)
    text(margin + 2 * mm, y - 22 * mm, customer.get("address") if customer else "-", 7, False, 84)
    text(margin + 112 * mm, y - 17 * mm, "INSCRICAO ESTADUAL", 5, True)
    text(margin + 112 * mm, y - 22 * mm, customer.get("state_registration") if customer else "-", 7, False, 25)
    text(margin + 152 * mm, y - 17 * mm, "PEDIDO / VENDEDOR", 5, True)
    text(margin + 152 * mm, y - 22 * mm, f"{invoice.get('order_number')} / {order.get('seller_name') if order else '-'}", 7, False, 32)
    y -= dest_h + 4 * mm

    text(margin, y, "CALCULO DO IMPOSTO", 7, True)
    y -= 2 * mm
    calc_h = 13 * mm
    col = usable / 6
    calc_values = [
        ("BASE ICMS", "0,00"),
        ("VALOR ICMS", "0,00"),
        ("VALOR PRODUTOS", br_money(subtotal)),
        ("DESCONTO", br_money(discount)),
        ("OUTRAS DESP.", "0,00"),
        ("VALOR TOTAL NF", br_money(total)),
    ]
    for index, (title, value) in enumerate(calc_values):
        label_box(margin + (index * col), y, col, calc_h, title, value, 7, index == 5)
    y -= calc_h + 4 * mm

    text(margin, y, "PRODUTOS / SERVICOS", 7, True)
    y -= 2 * mm
    table_x = margin
    row_h = 7 * mm
    widths = [16 * mm, 61 * mm, 18 * mm, 16 * mm, 14 * mm, 16 * mm, 21 * mm, 24 * mm]
    headers = ["CODIGO", "DESCRICAO", "NCM", "CFOP", "UN", "QTD", "UNITARIO", "TOTAL"]
    pdf.setFillColor(colors.lightgrey)
    pdf.rect(table_x, y - row_h, usable, row_h, fill=1, stroke=1)
    pdf.setFillColor(colors.black)
    x = table_x
    for width, title in zip(widths, headers):
        text(x + 1 * mm, y - 4.8 * mm, title, 5, True, 18)
        x += width
    y -= row_h
    ...
    for item in items[:28]:
        if y < 38 * mm:
            pdf.showPage()
            y = page_height - margin
        pdf.rect(table_x, y - row_h, usable, row_h, fill=0, stroke=1)
        x = table_x
        values = [
            str(item.get("code") or "")[:10],
            str(item.get("description") or "")[:42],
            _digits_only(item.get("ncm"))[:8],
            _digits_only(item.get("cfop"))[:4],
            str(item.get("unit") or "UN")[:4],
            money(item.get("quantity")),
            money(item.get("unit_price")),
            money(item.get("total")),
        ]
        for idx, (width, value) in enumerate(zip(widths, values)):
            if idx >= 5:
                right(x + width - 1 * mm, y - 4.8 * mm, value, 6)
            else:
                text(x + 1 * mm, y - 4.8 * mm, value, 6, False, 42)
            x += width
        y -= row_h

    y -= 5 * mm
    info_h = max(24 * mm, y - margin)
    box(margin, y, usable, min(info_h, 42 * mm), "Dados adicionais / Informacoes complementares")
    wrapped(
        margin + 2 * mm,
        y - 9 * mm,
        invoice.get("message") or invoice.get("sefaz_return") or "Documento gerado pelo Licensafe API.",
        130,
        6,
        5,
    )
    text(margin + 2 * mm, margin + 5 * mm, "DANFE gerado pelo Licensafe ERP - Documento auxiliar da NF-e autorizada.", 6)
    pdf.save()
...
def _parse_cora_invoice_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pix_copy_paste": pix.get("emv")
        or pix.get("copy_paste")
        or pix.get("copyPaste")
        or _find_first_string(data, ["emv", "copy_paste", "copyPaste", "pix_copy_paste"]),
    }
