...
def list_users():
    return fetch_all("select * from app_users where deleted_at is null and email not like 'vendedor.online%@licensafe.local' order by name asc")
...
def mobile_users(user: dict[str, Any] = Depends(require_seller_session)) -> list[dict[str, Any]]:
    ...
    return fetch_all(
        """
        select id, local_id, name, email, role, active, updated_at
        from app_users
        where deleted_at is null
          and email not like 'vendedor.online%@licensafe.local'
        order by name asc
        """
    )
