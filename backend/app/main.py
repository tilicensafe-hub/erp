...
def issue_mobile_billing(number: int, user: dict[str, Any] = Depends(require_seller_session)) -> dict[str, Any]:
    ...
    if existing:
        return {"ok": True, "message": "Boleto ja registrado para este pedido.", "items": existing}
    ...
    for index, days in enumerate(terms):
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Falha ao emitir boleto Cora: {error}") from error
        row = fetch_one(
            """
...
def _issue_cora_invoice(order: dict[str, Any], amount: float, due_date: str, days: int) -> dict[str, Any]:
    ...
    data = response.json()
    bank_slip = (data.get("payment_options") or {}).get("bank_slip") or {}
    pix = (data.get("payment_options") or {}).get("pix") or {}
    return {
        "id": data.get("id"),
        "code": data.get("code"),
...
def _cora_cert_tuple() -> tuple[str, str]:
    ...
    with open(cert_path, "w", encoding="utf-8") as cert_file:
        cert_file.write(settings.cora_certificate_pem.replace("\\n", "\n"))
    with open(key_path, "w", encoding="utf-8") as key_file:
        key_file.write(settings.cora_private_key_pem.replace("\\n", "\n"))
    return cert_path, key_path
