...
def _mobile_danfe_pdf(
    ...
    logo_space = 20 * mm if os.path.exists(logo_path) else 0
    if logo_space:
        try:
            pdf.drawImage(ImageReader(logo_path), margin + 3 * mm, y - 24 * mm, width=16 * mm, height=16 * mm, preserveAspectRatio=True, mask="auto")
        except Exception:
            logo_space = 0
    issuer_x = margin + logo_space
    issuer_w = left_w - logo_space
    center(issuer_x, y - 9 * mm, issuer_w, issuer, 9, True)
    center(issuer_x, y - 14 * mm, issuer_w, settings.nfe_issuer_trade_name or issuer, 7, False)
    wrapped(margin + 2 * mm, y - 20 * mm, settings.nfe_issuer_address, 54, 6, 2)
    text(margin + 2 * mm, y - 31 * mm, f"CNPJ: {doc_number(settings.nfe_issuer_cnpj)}", 6, False, 45)
    text(margin + 38 * mm, y - 31 * mm, f"IE: {_digits_only(settings.nfe_issuer_ie)}", 6, False, 35)
